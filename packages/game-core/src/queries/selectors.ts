...
import type { EngineSession, ActionOption, PhaseSummary } from "../contracts/engine";
...
function selectedTileLabel(state: GameState, tileId: string | null): string | undefined {
  return tileId == null ? undefined : state.content.actionTiles.find((tile) => tile.id === tileId)?.label;
}
...
export function selectPhaseSummary(session: EngineSession): PhaseSummary {
  ...
  return {
    ...
    actionLabel: selectedTileLabel(state, state.turn.selectedActionTiles[activePlayer.id] ?? null),
  };
}
