...
import { ACTION_TILE_DEFINITIONS, type ActionTileId } from "../data/setup/actionTiles";
...
function actionTilePriority(tileId: ActionTileId | null | undefined): number {
  return ACTION_TILE_DEFINITIONS.find((tile) => tile.id === tileId)?.value ?? 99;
}
...
function sortTurnOrder(players: readonly PlayerState[], selected: Record<string, ActionTileId | null>): string[] {
  return [...players]
    .sort((left, right) => {
      const leftPriority = actionTilePriority(selected[left.id]);
      const rightPriority = actionTilePriority(selected[right.id]);
...
function resetForBuildPhase(state: GameState): GameState {
  const buildOrder = sortTurnOrder(state.players, state.turn.selectedActionTiles);
  return {
    ...state,
    turn: {
      ...state.turn,
