...
function computeRotatedExits(tileId: string, rotation: number) {
  const manifest = TILE_MANIFEST.find((tile) => tile.id === tileId);
  if (!manifest) {
    return [];
  }
...
export function canPlaceTrack(options: {
  ...
  const manifest = TILE_MANIFEST.find((tile) => tile.id === options.tileId);
  if (!manifest) {
  ...
  const rotatedExits = computeRotatedExits(options.tileId, options.rotation);
